...
// 2. Aggiungi voce al menu di My Account
add_filter('woocommerce_account_menu_items', function($items) {
    // Ruoli abilitati
    $allowed = ['administrator', 'partner', 'affiliato'];
    if (wcm_user_has_role($allowed)) {
        // Inserisce "Commissioni" dopo "Dashboard"
        $pos = array_search('dashboard', array_keys($items));
        $out = array_slice($items, 0, $pos + 1, true) +
               ['commissioni' => __('Commissioni','wholesale-commission-manager')] +
...
// 3. Mostra i dati all'endpoint SOLO se admin o partner/affiliato
add_action('woocommerce_account_commissioni_endpoint', function() {
    ...
    // Usa lo stesso codice dello shortcode oppure chiama do_shortcode
    // echo do_shortcode('[wm_commissioni_advanced]');
    // O visualizza i dati in HTML tabellare:
...
// 4. Flush rewrite (solo una volta in attivazione plugin)
